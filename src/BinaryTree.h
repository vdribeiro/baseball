...
public:
  ...
  BTNode(const T &e, BTNode *esq, BTNode *dir)
      : element(e), left(esq), right(dir) { }
  BTNode(const T &e)
      : element(e), left(0), right(0) { }
  BTNode(const T &e, BTNode *esq)
      : element(e), left(esq), right(0) { }
};
...
class BinaryTree {
 public:
  ...
  BinaryTree(const T&elem, const BinaryTree & esq);
...
BinaryTree<T>::BinaryTree(const T&elem, const BinaryTree & esq)
{
  root = new BTNode<T>(elem, copySubtree(esq.root));
}
